...
45 นาที) หรือรัน `npm run gfm:backfill -w apps/etl` ในเครื่องแล้วอัปด้วย `rclone` เอง
คีย์ `flood/gfm/backfill-state.json` **ถูกลบออกจาก R2 แล้ว 2026-09-21** (ไม่มีใครอ่านหรือเขียนอีกหลังถอด Phase 2)
ฉากที่ backfill ลงไปแล้วยังอยู่ครบใน `aoi/{code}/flood/` และเสิร์ฟตามปกติ

**ผลที่ backfill ทำไปได้จริงก่อนถูกถอด** (จาก `backfill-state.json` ใบสุดท้าย + วัดจาก prod 2026-09-21 ก่อนลบ
ไฟล์ทิ้ง — เป็นบันทึกเดียวที่เหลือ) รันระหว่าง 2026-09-14T21:48Z → 2026-09-20T01:30Z แตะไป **12 จังหวัด**
(รหัส 10–21) จากทั้งหมด 77:

| จังหวัด | สถานะใน state | ฉากใน `index.json` | JSON ดิบ | **บนสาย (gz)** | ฉากเก่าสุด |
|---|---|---|---|---|---|
| 10, 11 | done | 735 / 732 | 467 / 464 KB | 40.2 / 39.1 KB | 2015-01-09 |
| 12 | done | 735 | 373 KB | 31.6 KB | 2015-01-09 |
| 13, 14 | done | 735 / 735 | 717 / 717 KB | 54.0 / 54.2 KB | 2015-01-09 |
| 15, 17 | done | 735 / 734 | 393 / 392 KB | 33.0 / 32.0 KB | 2015-01-09 |
| 18 | done | 985 | 526 KB | 43.8 KB | 2015-01-09 |
| 16 | **failed** (index ล้น) | 1,130 | 686 KB | 55.5 KB | 2019-04-10 |
| 19 | **failed** (index ล้น) | 945 | 574 KB | 46.0 KB | 2020-03-17 |
| 20 | **failed** (index ล้น) | 874 | 504 KB | 45.1 KB | 2020-08-09 |
| 21 | ค้างกลางทาง (`current`) | 1,340 | 675 KB | 55.7 KB | 2017-12-23 |
| 57 (ไม่โดน backfill, ไว้เทียบ) | — | 26 | 7.5 KB | 1.1 KB | — |

**สองคอลัมน์นี้ห้ามสลับกัน** "JSON ดิบ" คือสิ่งที่ `curl` เปล่า ๆ ได้ (ไม่ต่อรองการบีบอัด) และเป็นขนาดที่
เบราว์เซอร์ต้อง `JSON.parse` จริง ส่วน "บนสาย" คือสิ่งที่ผู้ใช้จริงโหลด เพราะเบราว์เซอร์ส่ง
`Accept-Encoding` เสมอ (วัด 2026-09-21: gz ตามตาราง, br เล็กกว่าเล็กน้อยที่ 55–61 KB)

รวมฉากที่**เก็บอยู่จริง** 10,415 ฉาก — ตัวเลข `provinces[].scenes` ใน state (รวม 33,075) คือจำนวนครั้งที่
เรียกประมวลผล นับรอบที่ล้มและทำซ้ำข้ามการเยี่ยมด้วย **ไม่ใช่**จำนวนฉากที่ได้ อย่าเอาไปอ้างเป็นปริมาณข้อมูล

16 / 19 / 20 ล้มด้วยเหตุเดียวกัน: `IndexOverflowError` — รวมแล้วเกินเพดาน `INDEX_MAX_SCENES` (1,500 ฉาก)
จึงปฏิเสธเขียน `index.json` ทั้งใบ (ตามออกแบบ: ไม่ตัดฉากเก่าทิ้งเงียบ ๆ) ฉากที่เกินมาไม่เคยถูกอัปขึ้น R2
เพราะ Phase 2 คัดลอกเฉพาะการเยี่ยมที่ exit 0/3 — **ไม่ได้ไล่ list บักเก็ตเพื่อยืนยัน** เป็นการอ้างจากสัญญาใน
workflow เท่านั้น ถ้าจะยืนยันต้อง `rclone lsjson` prefix นั้นสักครั้ง (Class A หนึ่งชุด)

**ต้องแยก index ไหม — ไม่ (วัดแล้ว 2026-09-21)** เกณฑ์ที่ F6 เขียนไว้เองคือ "แยก recent/archive เมื่อเกิน
~256 KB gz" จังหวัดที่หนักสุดตอนนี้คือ 21 ที่ **55.7 KB gz = 22% ของเกณฑ์** ยังห่างมาก การแยก index
จึงยังไม่มีเหตุผลรองรับ ส่วนเกณฑ์รับงานที่เขียนสั้น ๆ ว่า "ทุกจังหวัด index ต่ำกว่า ~300 KB" ไม่ได้ระบุหน่วย:
อ่านแบบ JSON ดิบคือเกิน 12 จังหวัด อ่านแบบบนสาย (หน่วยเดียวกับเกณฑ์แยก index) คือไม่เกินสักจังหวัด
— บันทึกไว้ทั้งสองแบบ ไม่เลือกอ่านแบบที่เข้าข้างตัวเอง

สิ่งที่ยังเป็นต้นทุนจริงคือฝั่ง client: `JSON.parse` ของ 373–717 KB และ array 735–1,340 ฉากต่อการเปิด
จังหวัดหนึ่งครั้ง (`max-age=300`) เทียบกับ 7.5 KB / 26 ฉากของจังหวัดที่ไม่โดน backfill ยังไม่มีการวัดว่า
กระทบเวลาเปิดจังหวัดจริงกี่ ms — ถ้าจะตัดสินใจเรื่องนี้ต้องวัดก่อน ไม่ใช่เดาจากขนาดไฟล์
...
ที่เคยผ่าน `devops` ไว้จึงไม่เกิดเต็มจำนวน — ค่าที่เกิดจริงคือ 10,415 ฉากที่ลงไปแล้วใน 12 จังหวัด (§1)
ซึ่งอยู่บน R2 ต่อไปและไม่ถูกลบ ≈ 0.9–1.6 GB (คิดที่ 85–151 KB/ฉาก ตามช่วงที่วัดได้ ไม่ได้วัดผลรวมจริง)
= **+$0.013–0.024 ต่อเดือนถาวร** บวก
...
code only, no `data/*.json`: the bundle is now 5,636,096 B upload / 861.57 KiB gzip. On the web side the entry + vendor
chunks are 356.88 kB gz of the 360 kB warning guard in `scripts/check-bundle-budget.mjs` (the Storm panel is a lazy chunk outside it)
