...
## 8. prefix `aoi/{code}/flood/` — ฉากน้ำท่วมจากดาวเทียม (E14)

ชั้นน้ำท่วมจาก Copernicus GFM (E14, `docs/methodology/flood-depth.md`) อยู่ใน prefix
ของตัวเองใต้จังหวัด **แยกจาก `v/{ver}`** ของหัวข้อ 7 โดยตั้งใจ: ไทล์ภูมิประเทศถูก
กำหนดที่อยู่ด้วย *รุ่นที่ปล่อย* (release-addressed — ไบต์ทั้งชุดเปลี่ยนพร้อมกันเมื่อ
build ใหม่) ส่วนฉากน้ำท่วมถูกกำหนดที่อยู่ด้วย *เวลา* (time-addressed — ทุกรอบบิน
ของ Sentinel-1 คือหนึ่งฉากใหม่ที่ไม่ทับฉากเก่า) ยัดฉากเข้าไปใต้รุ่นของชุดข้อมูล
= ต้องก๊อปฉากทั้งสิบปีทุกครั้งที่ปล่อยรุ่นภูมิประเทศใหม่ ซึ่งไม่มีเหตุผล
...
| `flood/gfm/backfill-state.json` | job เดียวกัน (Phase 2, E14.F6) — cursor ของการ backfill 2015→ปัจจุบัน `{version, floor, provinces: {code: {to, done, failed, failures, lastError, scenes}}, order, current, startedAt, updatedAt}` (`failures` = จำนวนการเยี่ยมที่ล้ม**ติดกัน** — `failed` ซึ่งทำให้จังหวัดถูกข้ามถาวรจะถูกตั้งเมื่อครบ 3 ครั้งติด ไม่ใช่ครั้งแรก เพราะ STAC ล่มชั่วคราวไม่ควรทิ้งช่วง 11 ปีของจังหวัดนั้น; การเยี่ยมที่สำเร็จรีเซ็ตตัวนับ) เขียนไม่เกินหนึ่งครั้งต่อการเยี่ยมจังหวัด **หลัง**ฉากของจังหวัดนั้นอัปขึ้น R2 สำเร็จเท่านั้น (copy ล้ม = state ไม่ขยับ job แดง); Phase 2 อัปเฉพาะ `aoi/{code}/flood` ของจังหวัดที่เยี่ยมด้วย `rclone copy --no-traverse --checksum` เหมือน Phase 1 และไม่แตะ `state.json`; `merge_index` **โยน `IndexOverflowError`** เมื่อจังหวัดเกิน `INDEX_MAX_SCENES` (1,500 ฉาก) แทนตัดฉากเก่าทิ้งเงียบ ๆ — index ของจังหวัดนั้นไม่ถูกเขียนและถูกลองใหม่ ไม่ใช่ฉากหายจาก UI โดยไม่มีใครรู้ | ไม่เสิร์ฟให้เบราว์เซอร์ — prefix `flood/gfm/` ไม่มี route ใน Worker |
...
เพราะเบราว์เซอร์ที่แคชไว้จะยิงชื่อเดิมได้อีกเต็มปี วิธีที่ถูกคือเพิ่มรุ่นของ layout
(`FLOOD_FIELD_VERSION`) หรือต่อท้าย `sceneId` ด้วยตัวบ่งชี้วิธีคำนวณ แล้วให้
`index.json` ชี้ไปที่ชื่อใหม่
...
บั๊กด้านต้นทุน ไม่ใช่ทางเลือกในการออกแบบ

`index.json` ถูกแคชสั้น (5 นาที + serve-stale อีก 10 นาที) เพราะมันเปลี่ยนเมื่อมีฉาก
ใหม่ราวทุก 6 ชั่วโมง และการเห็น index เก่าไป 5 นาทีคือเห็นฉากล่าสุดช้าไป 5 นาที
ไม่ใช่การเห็นข้อมูลผิด — ทุกฉากที่ index เก่าชี้ยังอยู่ครบ (immutable)

### ตัวอ่าน path

`apps/web/worker/floodPath.ts` (F3) ทำหน้าที่เดียวกับ `tilePath.ts`: รับเฉพาะรูปร่างที่รู้จัก
(`/aoi/(\d{2})/flood/(index\.json|(\d{8}T\d{6}-[A-Z]{2}\d{3}M)/(field\.bin|meta\.json))`)
แล้ว**ประกอบ**คีย์ R2 ขึ้นจากส่วนที่แมตช์ ไม่ใช่ส่งต่อ path ทั้งก้อน — traversal จึง
