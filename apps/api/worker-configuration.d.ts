/* eslint-disable */
// NOTE: secrets are stripped from this file by hand after every `wrangler types`
// run, so the hash below no longer matches the content — that is expected.
// `wrangler types` reads whatever sits in the local `.dev.vars` and declares it a
// REQUIRED `string`; `src/env.d.ts` declares the same secrets as optional on the
// same `__BaseEnv_Env` interface, and two members differing in optionality is a
// TS2717 declaration-merging error. TMD_UID/TMD_UKEY are already absent for this
// reason; TMD_NWP_TOKEN (E12.2) is the third. Re-strip after regenerating.
// Generated by Wrangler by running `wrangler types` (hash: 485b5dbd8cf76ba930b8ce287859b2f0)
...
interface __BaseEnv_Env {
	...
	ALLOWED_ORIGINS: "";
	EARTHQUAKE_FEED: DurableObjectNamespace<import("./src/index").EarthquakeFeedDO>;
	FORECAST_POINTER: DurableObjectNamespace<import("./src/index").ForecastPointerDO>;
	FORECAST_NWP: DurableObjectNamespace<import("./src/index").ForecastNwpDO>;
	OBSERVATION_CACHE: DurableObjectNamespace<import("./src/index").ObservationCacheDO>;
...
declare namespace NodeJS {
	interface ProcessEnv extends StringifyValues<Pick<Cloudflare.Env, "EQ_DEFAULT_BBOX" | "ALLOWED_ORIGINS">> {}
}
