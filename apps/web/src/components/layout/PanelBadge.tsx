...
/**
 * badge ของปุ่มแผง — สี่สถานะเดียวกับ `AlertRailBadge` บวก `title` ที่แผงอื่นใส่แทน
 * ข้อความของแจ้งเตือน อปท. ได้ (แผงพายุ: "N พายุ" ไม่ใช่ "N รายการที่ active")
 */
export type RailBadge = (NonNullable<AlertRailBadge> & { title?: string }) | null;
 ...
 */
export function PanelBadge({ badge }: { badge: RailBadge }) {
  const t = useT();
  ...
  if (badge.kind === "count") {
    return (
      <span
        ...
        title={badge.title ?? t("alert.banner.count", { n: badge.n })}
      >
        {badge.n}
      </span>
    );
  ...
  if (badge.kind === "neverEvaluated") {
    return (
      <span
        ...
        title={badge.title ?? t("alert.badge.neverEvaluated")}
      >
        ?
      </span>
    );
  ...
  return (
    <span
      ...
      title={badge.title ?? (unreachable ? t("alert.banner.unreachable") : t("alert.banner.degraded"))}
    />
  );
}
