...
export const en: Record<keyof typeof th, string> = {
  ...
  "popup.source": "Source",
  "popup.camera.fallbackName": "Camera {id}",
  "popup.camera.nearest": "Nearby camera ({km} km) — {source}",
  "popup.camera.open": "View",
  "popup.camera.owner": "Camera: {org}",
  "popup.camera.rights": "Footage belongs to the camera owner / source named here — none of them endorses or is affiliated with this project",
  ...
  "popup.camera.streams": "Streams of this camera",
  "popup.camera.kindVideo": "Video",
  "popup.camera.kindStill": "Still image",
  "popup.camera.kind.dwrSnapshot": "Snapshot",
  "popup.camera.kind.dwrMjpeg": "Live",
  "popup.camera.kind.hls": "Video",
  "popup.camera.kind.mjpeg": "Live image",
  "popup.camera.kind.jpeg": "Still image",
  "popup.camera.kind.jpegFetch": "Still image",
  "stream.dwrSnapshot.loading": "Requesting the latest image from DWR…",
  "stream.dwrSnapshot.alt": "Latest image from camera {name}",
  "stream.dwrSnapshot.takenAt": "Taken",
  "stream.dwrSnapshot.takenAtUnknown": "Unknown (DWR did not give it in a readable form)",
  "stream.dwrSnapshot.fetchedAt": "Image fetched",
  "stream.dwrSnapshot.age": "Image age",
  "stream.dwrSnapshot.stale": "Not a current image",
  "stream.dwrSnapshot.old": "Older than 24 h",
  ...
  "stream.dwrSnapshot.noImage": "DWR answered but has no image from this camera — this says nothing about conditions here",
  "stream.dwrSnapshot.refresh": "New image",
  "stream.dwrSnapshot.badge": "Snapshot · taken {time}",
  "stream.dwrSnapshot.badgeNoTime": "Snapshot · capture time unknown",
  "stream.mjpeg.alt": "Live image from camera {name}",
  "stream.mjpeg.connecting": "Connecting… (MJPEG live images take about 4–6 s to send the first frame)",
  "stream.mjpeg.failed":
    "No live image from this camera — the source sent none or we could not connect (the browser does not say which), so nothing can be said about this camera or this area",
  "stream.mjpeg.liveBadge": "LIVE",
  "stream.mjpeg.pausedBadge": "Live paused",
  "stream.mjpeg.reconnectingBadge": "Reconnecting…",
  "stream.mjpeg.reconnecting":
    "No new frame from the source for a while, reconnecting — the picture is the last frame received, not a live one",
  "stream.mjpeg.paused":
    "Live view stops by itself after 5 min so the source's server is not held open — the picture is the last frame received, not a current one",
  "stream.mjpeg.noTime": "MJPEG live image (reconnects every ~15 s) — the stream carries no capture time, so none is shown",
  "stream.mjpeg.resume": "Resume live",
  "stream.mjpeg.retry": "Try again",
  "stream.hls.videoLabel": "Video from camera {name}",
  "stream.hls.loading": "Connecting to the video stream…",
  "stream.hls.suspended":
    "The source has no stream from this camera right now ({detail}) — it may be suspended; this says nothing about the road or the area",
  "stream.hls.unreachable":
    "Could not open the stream from the video server ({detail}) — the network failed or the server answered in a way the browser may not read (the two cannot be told apart), so nothing can be said about this camera or this area",
  "stream.hls.unsupported": "This browser cannot play this video ({detail}) — this says nothing about the camera or the area",
  "stream.hls.streamTime": "Time in stream",
  "stream.hls.noStreamTime": "This stream carries no timestamp (no EXT-X-PROGRAM-DATE-TIME), so no capture time is shown",
  "stream.hls.retry": "Try again",
  "stream.hls.liveBadge": "LIVE",
  "stream.hls.bufferingBadge": "Buffering…",
  "stream.hls.buffering": "The video is waiting for data from the source — the frozen frame is not live; this says nothing about the camera or the area",
  "stream.hls.pausedBadge": "Paused",
  "stream.hls.paused": "Paused — the frame shown is not live; pressing play jumps back to the live edge",
  "stream.jpeg.alt": "Latest still image from camera {name}",
  "stream.jpeg.loading": "Asking the image server for a still image…",
  "stream.jpeg.detail.error": "error",
  "stream.jpeg.detail.timeout": "timed out",
  "stream.jpeg.detail.rejected": "image link rejected",
  "stream.jpeg.unreachable":
    "Could not get an image from the image server ({detail}) — either the network failed or the source answered without an image (the two cannot be told apart), so this says nothing about the camera or the area; retrying",
  "stream.jpeg.paused": "Automatic refresh stopped after {min} minutes of viewing — the image shown may not be the latest",
  "stream.jpeg.pausedBadge": "Refresh stopped",
  "stream.jpeg.badge": "Still image, refreshed automatically",
  "stream.jpeg.fetchedAt": "Last image received",
  "stream.jpeg.neverFetched": "No image yet",
  "stream.jpeg.captureTime":
    "This camera provides still images, not video; we ask for a new one every ~{s} s while it is open — the capture time (if any) is burned into the picture and is not available as data, so the time above is when your browser received the image, not when it was taken",
  "stream.jpeg.resume": "Resume refresh",
  "stream.jpegFetch.alt": "Latest still image from camera {name}",
  "stream.jpegFetch.loading": "Requesting a still image from the source…",
  "stream.jpegFetch.unreachable":
    "Could not get an image from the source ({detail}) — the network failed or the source does not allow cross-origin reads, so nothing can be said about this camera or this area",
  "stream.jpegFetch.noImage": "The source answered but has no image from this camera — this says nothing about conditions here",
  "stream.jpegFetch.badge": "Still image · taken {time}",
  "stream.jpegFetch.badgeNoTime": "Still image · capture time unknown",
  "stream.jpegFetch.takenAt": "Taken (source's Last-Modified)",
