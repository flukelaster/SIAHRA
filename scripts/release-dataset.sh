...
# immutable หนึ่งปีทำให้แก้ไม่ได้ ซึ่งเป็นความผิดพลาดที่ E9.1/E9.2 มีไว้กันโดยตรง แต่ธงห้ามกัน
# ได้เฉพาะกรณีที่ build ใหม่ **ระหว่าง** รันสคริปต์นี้ ถ้า build ใหม่เกิดขึ้น**ก่อน**รัน (หรือ
# prefix เดิมถูกแก้โดยไม่ผ่านสคริปต์นี้) การห้ามใช้ธงร่วมกันเฉย ๆ ตรวจไม่เจอ — ขั้นก๊อปจริง
# ใน upload-tiles.sh จึงเทียบไบต์ในเครื่องนี้กับ prefix เดิมบน R2 ด้วย `rclone check` ก่อน
