...
export interface HazardLayerDescriptor {
  ...
  forecast?: {
     ...
     *
     * `null` = ต้นทางไม่ได้ประกาศไว้ในที่ที่ตรวจสอบย้อนได้ — ห้ามเติมจากตัวเลข
     * ที่เราเชื่อว่าน่าจะใช่ กรณีจริงคือ TMD NWP: ตัว API ไม่ส่ง metadata ของกริด
     * มาเลย (วัดจริง 2026-08-23 ทั้งบอดี้และ header) เหลือแต่หน้าเอกสารของ TMD
     * ซึ่งวันเดียวกันนั้นพิสูจน์แล้วว่าผิดสองจุด (คีย์ `WeatherForcasts` และรูป
     * `weather_forecast.locations[]` ของ daily ที่ไม่มีอยู่จริง) ตัวเลขจากหน้าเดียวกัน
     * จึงยังไม่ถือว่าตรวจสอบแล้ว
     */
