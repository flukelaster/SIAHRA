...
| `GET /api/v1/local-authorities/:id/exposure` | Baseline exposure (WorldPop 2020 population, OSM buildings/roads/facilities) for the 481 units with a real boundary (431 อปท. + 50 Bangkok districts) — static-reference, baked into the build |
...
| Tropical-cyclone tracks, western North Pacific + South China Sea (Storm panel, credited in the panel) | Japan Meteorological Agency (JMA), RSMC Tokyo – Typhoon Center (`jma.go.jp/bosai`) | [Public Data License (Version 1.0)](https://www.jma.go.jp/jma/en/copyright.html), compatible with CC BY 4.0 — the mandatory citation "Source: Japan Meteorological Agency website (https://www.jma.go.jp/)" plus a notice that SIAHRA re-plotted the track, positions and 70 % probability circles and that the edited map was not created by the Government of Japan |
| Tropical-cyclone tracks, North Indian Ocean (Storm panel, credited in the panel) | GDACS (EC-JRC / UN-OCHA); storm data from JTWC | No reuse licence published — credited "GDACS (EC-JRC / UN-OCHA), data: JTWC" with GDACS's [Disclaimer and Terms of Use](https://www.gdacs.org/About/termofuse.aspx) quoted verbatim next to the data |
| Storm panel basemap (country outlines) | Natural Earth 1:50m Admin 0 – Countries (v5.1.2) | Public domain — credited anyway |
| Live road-camera video (HLS, played on click, credited in the popup and the attribution line) | iTIC Foundation (มูลนิธิ iTIC) streams; cameras owned by the Department of Highways (DOH, กรมทางหลวง) and iTIC partner municipalities; camera list from Longdo (`camera.longdo.com`) | No licence granted — displayed with attribution to iTIC, the camera owners and Longdo (Longdo's API terms restrict redisplay; the owner accepted that risk on 2026-09-26) |
